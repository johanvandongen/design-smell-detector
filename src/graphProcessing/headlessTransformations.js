...
export const aggregateLayers = function (pCy) {
	...
	structures.forEach((clasz) => {
		...
		layers.push(...methods.map((method) => method.outgoers(e => edgeHasLabel(e, 'implements') && e.target().data('properties.kind')?.toLowerCase().includes("layer")).targets().data('properties.simpleName'))
